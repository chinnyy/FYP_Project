...
asv_sample_raw<-read_tsv(file = paste0(wd,'/DATA/METAPR2/woa_metapr2_ASVs_selected_abundance_Eukaryota.tsv'))
...
ggsave(filename=paste0(wd,"/PLOTS/R/P1/main_map.pdf"),width=40,height=19,units=c("cm"),bg="white")
...
for (i in 1:nrow(cord_df)){
  ...
  ggsave(filename=paste0(wd,"/PLOTS/R/P1/small_plot",cord_df[i,"plot_num"],".pdf"),dpi=300,bg="white")
}
...
p2_plot<- function(variable){
  ...
  ggsave(filename=paste0(wd,"/PLOTS/R/P2/Laby_scatterpie_",variable,"_",i,".pdf"),width=40,height=22,units=c("cm"),bg="white")
  ...
  for (i in unique(laby_order_sub[,variable])){
    ...
    ggsave(filename=paste0(wd,"/PLOTS/R/P2/Laby_treemap_",variable,"_",i,".pdf"),width=40,height=40,units=c("cm"),bg="white")
...
laby_depth<- asv_sample %>%  # for depth_level
  mutate(across(depth_level, factor, levels=c("surface","euphotic","pelagic")))%>%
  filter(ecosystem == "oceanic" & class == "Labyrinthulomycetes") %>% # Filter for laby and oceanic samples
...
ggsave(filename=paste0(wd,"/PLOTS/R/P2/Laby_depth_plot.pdf"),width=40,height=40,units=c("cm"),bg="white")
